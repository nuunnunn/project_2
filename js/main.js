$(document).ready(function () {
  $(window).scroll(function () {
    ...
    if (currentScrollTop > banner - 700) {
      ...
      $("#artifour p")
        .css("transform", "translateX(300px)")
        .css("opacity", "0");
    }
  ...
  if (window.matchMedia("(min-width: 1500px)").matches) {
    ...
    $(".next").click(function () {
      $(".slide")
        .stop()
        .animate({ marginLeft: "-25%" }, 800, function () {
          $(".slide li:first").appendTo(".slide");
          $(".slide").css({ marginLeft: 0 });
        });
    });
  } else if (
    window.matchMedia("(min-width:1300px) and (max-width:1499px)").matches
  ) {
    $(".prev").click(function () {
      $(".slide li:last").prependTo(".slide");
    ...
    $(".next").click(function () {
      $(".slide")
        .stop()
        .animate({ marginLeft: "-24%" }, 800, function () {
          $(".slide li:first").appendTo(".slide");
          $(".slide").css({ marginLeft: 0 });
        });
    });
  } else if (
    window.matchMedia("(min-width:980px) and (max-width:1299px)").matches
  ) {
    $(".prev").click(function () {
      $(".slide li:last").prependTo(".slide");
    ...
    $(".next").click(function () {
      $(".slide")
        .stop()
        .animate({ marginLeft: "-32%" }, 800, function () {
          $(".slide li:first").appendTo(".slide");
          $(".slide").css({ marginLeft: 0 });
        });
    });
  } else if (
    window.matchMedia("(min-width:600px) and (max-width:979px)").matches
  ) {
    $(".prev").click(function () {
      $(".slide li:last").prependTo(".slide");
    ...
    $(".next").click(function () {
      $(".slide")
        .stop()
        .animate({ marginLeft: "-50%" }, 800, function () {
          $(".slide li:first").appendTo(".slide");
          $(".slide").css({ marginLeft: 0 });
        });
    });
  ...
  //hover 텍스트 밑 라인 움직임
  var linewidht = $("#artithr p:last-of-type").width();
  var linewidht2 = $("#artifour p:last-of-type").width();
  $("#artithr").hover(
    function () {
      $(this).children(".line").stop().animate({ width: linewidht }, 1000);
    },
    function () {
  ...
  $("#artifour").hover(
    function () {
      $(this).children(".line").stop().animate({ width: linewidht2 }, 1000);
    },
    function () {
      $(this).children(".line").stop().stop().animate({ width: "0%" }, 1000);
    }
